...
function formatShareLabel(shares: ProfileShareWithUser[]): string {
  const names = shares.map(
    (item) => item.shared_with_name?.trim() || item.shared_with_email.split("@")[0],
  );
  if (names.length === 1) return `Compartido con ${names[0]}`;
  if (names.length === 2) return `Compartido con ${names[0]} y ${names[1]}`;
  return `Compartido con ${names[0]} y ${names.length - 1} más`;
}

function formatOwnerLabel(ownerName: string | null | undefined, ownerEmail?: string): string {
  const name = ownerName?.trim();
  if (name) return `Compartido por ${name}`;
  if (ownerEmail) return `Compartido por ${ownerEmail.split("@")[0]}`;
  return "Compartido con vos";
}
...
export function ProfileCard({
  ...
  const permissions = share ?? null;
  const canView = isOwner || Boolean(permissions?.can_view_profile || permissions?.can_edit_profile);
  const canEdit = isOwner || Boolean(permissions?.can_edit_profile);
  const canHealthBook =
    isOwner || Boolean(permissions?.can_view_health_book || permissions?.can_edit_profile);
  const canSaveLoc = isOwner || Boolean(permissions?.can_save_location);
  const alertsOnly =
    !isOwner &&
    Boolean(permissions?.can_receive_alerts) &&
    !canView &&
    !canEdit &&
    !canHealthBook &&
    !canSaveLoc;
      ...
      >
        ...
        >
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2 pr-10">
            <span
              className={cn(
                "inline-flex items-center gap-2 rounded-full py-1 pl-1.5 pr-3.5 text-xs font-bold uppercase tracking-wide",
                theme.badge,
              )}
            >
            ...
            {isOwner && shares.length > 0 && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-violet-800">
                <Users className="h-3 w-3" aria-hidden />
                {formatShareLabel(shares)}
              </span>
            )}
            {!isOwner && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-sky-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-sky-900">
                <Share2 className="h-3 w-3" aria-hidden />
                {formatOwnerLabel(ownerName, ownerEmail)}
              </span>
            )}
            {alertsOnly && (
            ...
            {!isOwner && share?.expires_at && (
              <span className="rounded-full bg-neutral-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-neutral-700">
                Vence {new Date(share.expires_at).toLocaleDateString("es-AR")}
              </span>
            )}
          </div>
          ...
          <div className="flex items-start gap-4 pr-8">
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              ...
              {alertsOnly && (
                <p className="mt-2 text-sm leading-snug text-neutral-500">
                  Recibís alertas push cuando alguien escanea este QR.
                </p>
              )}
            </div>
          </div>
